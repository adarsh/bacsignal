  ...
  success: function(position) {
    ...
    function dishesIndex(event, data){
      _.each(data, function(dish) {
        var miles = locationFinder(dish.locations, latitude, longitude);
...
function locationFinder (locations, myLat, myLng) {
  var miles = distance (myLat, myLng, locations[0].lat, locations[0].lng)

  _.each(locations, function (place) {
    var place_distance = distance (myLat, myLng, place['lat'], place['lng'])
